...
export default function AdminSettings() {
    ...
    const fetchSettings = async () => {
        try {
            ...
            settingsData.forEach(setting => {
                ...
                if (setting.key === 'site_favicon_url') {
                ...
                await settingService.bulkUpdate([
                    {
                        ...
                        description: 'URL of the site favicon (browser tab icon)'
                    },
                    {
                        key: 'site_favicon_id',
                        value: uploadedImage.id.toString(),
                        ...
                        group: 'general',
                        subsection: 'branding',
                        label: 'Site Favicon Media ID',
                        description: 'Media ID of the site favicon'
                    }
                ]);

                setFaviconUrl(uploadedImage.url);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow">
                <h2 className="text-xl font-bold mb-4 text-gray-800 border-b pb-3">Site Favicon (Browser Tab Icon)</h2>
                <div className="flex items-start gap-6">
                    <div className="flex-shrink-0">
                        {faviconUrl ? (
                            <img
                                src={faviconUrl}
                                alt="Site Favicon"
                                className="w-16 h-16 object-contain border-2 border-gray-200 rounded-lg p-2 bg-gray-50"
                            />
                        ) : (
                    ...
                    <div className="flex-1">
                        <h3 className="font-semibold text-gray-900 mb-2">Upload Favicon</h3>
                        <p className="text-sm text-gray-600 mb-4">
                            Upload a favicon to display in the browser tab. This is the small icon that appears next to your site title.
                            Recommended size: 32x32px or 64x64px. Supported formats: ICO, PNG, SVG, JPG.
                        </p>
                        <div className="flex gap-3">
                            <button
                                ...
                                <button
                                    onClick={async () => {
                                        if (confirm('Are you sure you want to remove the favicon?')) {
                                            try {
                                                const faviconUrlSetting = settings.find(s => s.key === 'site_favicon_url');
                                                const faviconIdSetting = settings.find(s => s.key === 'site_favicon_id');

                                                const updatePayload = [];
                                                if (faviconUrlSetting) {
                                                    updatePayload.push({
                                                        key: 'site_favicon_url',
                                                        value: '',
                                                        type: faviconUrlSetting.type,
                                                        group: faviconUrlSetting.group,
                                                        subsection: faviconUrlSetting.subsection,
                                                        label: faviconUrlSetting.label,
                                                        description: faviconUrlSetting.description,
                                                    });
                                                }
                                                if (faviconIdSetting) {
                                                    updatePayload.push({
                                                        key: 'site_favicon_id',
                                                        value: '',
                                                        type: faviconIdSetting.type,
                                                        group: faviconIdSetting.group,
                                                        subsection: faviconIdSetting.subsection,
                                                        label: faviconIdSetting.label,
                                                        description: faviconIdSetting.description,
                                                    });
                                                }
